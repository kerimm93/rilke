...
- [ ] In DevTools → Application → Cache Storage ausschließlich den zum aktuellen Worker-Pfad gehörenden Rilke-Cache leeren oder löschen; er beginnt mit `rilke-app-` und endet in Version A mit `-v1`. Den Worker nicht abmelden.
...
1. **Version A vorbereiten**
   - [ ] Mit `CACHE_VERSION = 'v1'` online laden.
   - [ ] Eine eindeutig benannte Karte anlegen und Reload-Persistenz prüfen.
   - [ ] In Cache Storage den pfadgebundenen Namen dokumentieren, zum Beispiel `rilke-app-%2Frilke%2F-v1` für ein Deployment unter `/rilke/`.
   - [ ] Wenn möglich eine zweite Test-App oder ein zweites Rilke-Deployment unter einem anderen Pfad derselben Origin laden und dessen abweichenden Cache-Namen dokumentieren.
   - [ ] Optional zusätzlich einen eindeutig fremden Cache anlegen oder vorhandenen fremden Cache-Namen dokumentieren.

2. **Version B bereitstellen**
   - [ ] Eine sichtbare, rein statische Teständerung vornehmen.
   - [ ] In `sw.js` ausschließlich für den Test `CACHE_VERSION` auf `v2` erhöhen.
...
3. **Normalen Lifecycle abschließen**
   ...
   - [ ] Der eigene pfadgebundene `-v1`-Cache wurde entfernt und der entsprechende `-v2`-Cache ist vorhanden.
   - [ ] Der Cache eines zweiten Deployments unter einem anderen Pfad derselben Origin wurde nicht gelöscht.
...
- Cache-Namen und Cleanup sind an den Worker-/Repository-Pfad gebunden; ältere Versionen anderer Deployments auf derselben Origin werden nicht gelöscht.
