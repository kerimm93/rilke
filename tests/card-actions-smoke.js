...
const context = {
  ...
  openModal(){ delete overlay.dataset.preserveTextWhitespace; },
  closeModal(){ delete overlay.dataset.preserveTextWhitespace; },
  renderCards(){}, returnCardModalFocusToTree(){},
...
assert.equal(context.nextOrder('base'), 1);
assert.equal(saves, 1);

// Child split canonicalizes historical leading order gaps before appending.
const gapSplitParent = card('base','1',null,0,'parent left and right');
const gapChildA = card('gap-child-a','1.a','base',1,'A');
const gapChildB = card('gap-child-b','1.b','base',2,'B');
gapSplitParent.childUids = ['gap-child-a','gap-child-b'];
reset([gapSplitParent,gapChildA,gapChildB]);
prepareSplit('parent left and right', 12);
comment.value = 'split parent comment';
status.value = 'active';
const gapSplitChild = context.splitCardAtCursor('base','child');
const canonicalChildren = context.childrenOf('base');
assert.equal(canonicalChildren.length, 3);
assert.deepEqual(Array.from(canonicalChildren, item=>item.order), [0,1,2]);
assert.deepEqual(Array.from(canonicalChildren, item=>item.sid), ['1.a','1.b','1.c']);
assert.equal(new Set(Array.from(canonicalChildren, item=>item.sid)).size, 3);
assert.equal(gapSplitChild.order, 2);
assert.equal(gapSplitChild.sid, '1.c');
assert.equal(gapSplitChild.parentUid, 'base');
assert.equal(gapSplitChild.text, 'and right');
assert.equal(context.findCard('base').text, 'parent left ');
assert.equal(context.nextOrder('base'), 3);
assert.equal(canonicalChildren.filter(item=>item.order === context.nextOrder('base')).length, 0);
assert.equal(gapChildA.updatedAt, gapSplitChild.updatedAt);
assert.equal(gapChildB.updatedAt, gapSplitChild.updatedAt);
assert.equal(context.findCard('base').updatedAt, gapSplitChild.updatedAt);
assert.equal(context.findCard('base').childUids.filter(uid=>uid === gapSplitChild.uid).length, 1);
assert.equal(saves, 1);

// The immediate post-split editor preserves leading boundary whitespace on save.
reset([card('base','1',null,0,'old'), card('tail','2',null,1,'tail')]);
const leadingRaw = 'left right';
prepareSplit(leadingRaw, 4);
const leadingCreated = context.splitCardAtCursor('base','sibling');
assert.equal(context.findCard('base').text, 'left');
assert.equal(leadingCreated.text, ' right');
assert.equal(overlay.dataset.preserveTextWhitespace, 'true');
editor.value = leadingCreated.text;
comment.value = leadingCreated.comment;
status.value = leadingCreated.status;
context.doEditCard(leadingCreated.uid);
assert.equal(leadingCreated.text, ' right');
assert.equal(context.findCard('base').text + leadingCreated.text, leadingRaw);
assert.equal(overlay.dataset.preserveTextWhitespace, undefined);

// Trailing boundary whitespace is likewise retained by that one split continuation session.
reset([card('base','1',null,0,'old'), card('tail','2',null,1,'tail')]);
const trailingRaw = 'left right  ';
prepareSplit(trailingRaw, 5);
const trailingCreated = context.splitCardAtCursor('base','sibling');
assert.equal(context.findCard('base').text, 'left ');
assert.equal(trailingCreated.text, 'right  ');
editor.value = trailingCreated.text;
comment.value = trailingCreated.comment;
status.value = trailingCreated.status;
context.doEditCard(trailingCreated.uid);
assert.equal(trailingCreated.text, trailingRaw.slice(5));
assert.equal(context.findCard('base').text + trailingCreated.text, trailingRaw);

// A leading LF is restored into the live textarea after HTML parsing and survives save.
reset([card('base','1',null,0,'old'), card('tail','2',null,1,'tail')]);
const newlineRaw = 'left\n\nright';
prepareSplit(newlineRaw, 4);
const newlineCreated = context.splitCardAtCursor('base','sibling');
assert.equal(context.findCard('base').text, 'left');
assert.equal(newlineCreated.text, '\n\nright');
assert.equal(editor.value, '\n\nright');
comment.value = newlineCreated.comment;
status.value = newlineCreated.status;
context.doEditCard(newlineCreated.uid);
assert.equal(newlineCreated.text, '\n\nright');
assert.equal(context.findCard('base').text + newlineCreated.text, newlineRaw);

// A later ordinary edit does not inherit the transient split whitespace mode.
const ordinary = context.findCard('tail');
context.openEditCardModal(ordinary.uid);
assert.equal(overlay.dataset.preserveTextWhitespace, undefined);
editor.value = '  geändert  ';
comment.value = ordinary.comment;
status.value = ordinary.status;
context.doEditCard(ordinary.uid);
assert.equal(ordinary.text, 'geändert');
...
const failedSplitSelection = [editor.selectionStart, editor.selectionEnd];
assert.equal(context.splitCardAtCursor('base','sibling'), null);
assert.equal(JSON.stringify(context.S), failedSplitSnapshot);
assert.equal(context.findCard('base').text, 'original complete text');
assert.equal(context.findCard('base').comment, 'stored comment');
assert.equal(context.findCard('base').status, 'draft');
assert.equal(context.findCard('base').order, 5);
assert.equal(context.findCard('base').sid, 'legacy-child');
assert.equal(context.findCard('base').updatedAt, '2025-01-01T00:00:00.000Z');
assert.equal(context.S.cards.length, 2);
assert.equal(saves, 0);
assert.equal(overlay.dataset.cardModal, 'edit');
assert.equal(overlay.dataset.cardUid, 'base');
assert.deepEqual([editor.selectionStart, editor.selectionEnd], failedSplitSelection);
